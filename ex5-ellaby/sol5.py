...
Original file is located at
    https://colab.research.google.com/drive/1lK4J-zVlLUaszOO49chU5dPUVuTnuw49
...
cs_id = "ellaby"  # @param {type:"string"}
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    images_cache = dict()
    while True:
        clean_batch, corrupted_batch = [], []
        for i in range(batch_size):
            img_path = filenames[np.random.randint(0, len(filenames))]  # random generate image
            if img_path in images_cache:
                img = images_cache[img_path]
            else:
                img = read_image(img_path, 1)
                images_cache[img_path] = img
            top_y = np.random.randint(0, img.shape[0] - 3 * crop_size[0])
            left_x = np.random.randint(0, img.shape[1] - 3 * crop_size[1])
            clean_patch = img[top_y + crop_size[0]: top_y + (2 * crop_size[0]),
                          left_x + crop_size[1]: left_x + (2 * crop_size[1])] - 0.5
            bigger_patch_currapted = corruption_func(
                img[top_y: top_y + (3 * crop_size[0]), left_x: left_x + (3 * crop_size[1])])
            currapted_patch = bigger_patch_currapted[crop_size[0]: 2 * crop_size[0],
                              crop_size[1]: 2 * crop_size[1]] - 0.5
            clean_batch.append(clean_patch)
            corrupted_batch.append(currapted_patch)
        yield (np.array(corrupted_batch)[..., np.newaxis], np.array(clean_batch)[..., np.newaxis])
...
def build_nn_model(height, width, num_channels, num_res_blocks):
    ...
    outputs = Conv2D(num_channels, (3, 3), padding='same')(inputs)
    outputs = Activation('relu')(outputs)
    for i in range(num_res_blocks):
        outputs = resblock(outputs, num_channels)
...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    """
    train_images, validation_images = train_test_split(images, train_size=0.8)
    train_data_generator = load_dataset(train_images, batch_size, corruption_func, model.input.shape[1:3])
    validation_data_generator = load_dataset(validation_images, batch_size, corruption_func, model.input.shape[1:3])
    model.compile(loss='mean_squared_error', optimizer=Adam(beta_2=0.9))
    model.fit_generator(generator=train_data_generator, steps_per_epoch=steps_per_epoch, epochs=num_epochs,
                        validation_data=validation_data_generator, validation_steps=num_valid_samples)
...
def restore_image(corrupted_image, base_model):
    ...
    clean_image = new_model.predict(corrupted_image)[0].astype(np.float64) + 0.5
    return np.clip(clean_image, 0, 1).squeeze()
...
def add_gaussian_noise(image, min_sigma, max_sigma):
    ...
    img_with_noise = image + noise
    img_with_noise = np.floor(255 * img_with_noise) / 255
    return np.clip(img_with_noise, 0, 1)
...
def learn_denoising_model(denoise_num_res_blocks, quick_mode=False):
    ...
    """
    patch_size, num_channels, min_sigma, max_sigma = (24, 24), 48, 0, 0.2
    [batch_size, steps_per_epoch, num_epochs, num_valid_samples] = [100, 100, 10, 1000] if not quick_mode else [10, 3,
                                                                                                                2, 30]
    images = images_for_denoising()
    corruption_func = lambda image: add_gaussian_noise(image, min_sigma, max_sigma)

    model = build_nn_model(patch_size[0], patch_size[1], num_channels, denoise_num_res_blocks)
    train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples)
    return model


def check_denoise():
    img = read_image(images_for_denoising()[1], 1)
    plt.imshow(img)
    plt.show()
    corrupted_image = add_gaussian_noise(img, 0, 0.2)
    plt.imshow(corrupted_image)
    plt.show()
    denoise_model = learn_denoising_model(5, quick_mode=False)
    restored_img = restore_image(corrupted_image, denoise_model)
...
def add_motion_blur(image, kernel_size, angle):
    ...
    blurred = convolve(image, kernel, mode="constant")
    blurred = np.floor(255 * blurred) / 255
...
def random_motion_blur(image, list_of_kernel_sizes):
    ...
    kernel_size = list_of_kernel_sizes[np.random.randint(0, len(list_of_kernel_sizes))]
...
def learn_deblurring_model(deblur_num_res_blocks, quick_mode=False):
    ...
    :return: the trained model
    """
    patch_size, num_channels, kernel_size = (16, 16), 32, 7
    [batch_size, steps_per_epoch, num_epochs, num_valid_samples] = [100, 100, 10, 1000] if not quick_mode else [10, 3,
                                                                                                                2, 30]
    images = images_for_deblurring()
    corruption_func = lambda image: random_motion_blur(image, [kernel_size])

    model = build_nn_model(patch_size[0], patch_size[1], num_channels, deblur_num_res_blocks)
    train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples)
    return model


def check_deblurr():
    img = read_image(images_for_deblurring()[1], 1)
    plt.imshow(img)
    plt.show()
    corrupted_image = random_motion_blur(img, [7])
    plt.imshow(corrupted_image)
    plt.show()
    deblur_model = learn_deblurring_model(5, quick_mode=False)
    restored_img = restore_image(corrupted_image, deblur_model)
    plt.imshow(restored_img)
    plt.show()
...
def super_resolution_corruption(image):
    ...
    corrupted = ndimage.zoom(image, float(1 / factor), prefilter=False)
    corrupted = ndimage.zoom(corrupted, factor, prefilter=False)
    rows, cols = np.meshgrid(np.arange(image.shape[0]), np.arange(image.shape[1]))
    corrupted = ndimage.map_coordinates(corrupted, [rows, cols], mode="mirror")
    return corrupted.transpose()
...
steps_per_epoch = 1500  # @param {type:"slider", min:100, max:5000, step:100}
num_epochs = 10  # @param {type:"slider", min:1, max:20, step:1}
patch_size = 32  # @param {type:"slider", min:8, max:32, step:2}
num_channels = 50  # @param {type:"slider", min:16, max:64, step:2}
...
def learn_super_resolution_model(super_resolution_num_res_blocks, quick_mode=False):
    ...
    :return: the trained model
    """
    [sr_batch_size, sr_steps_per_epoch, sr_num_epochs, sr_num_valid_samples] = \
        [batch_size, steps_per_epoch, num_epochs, 1000] if not quick_mode else [10, 3, 2, 30]
    ...
    train_model(model, images, corruption_func, sr_batch_size, sr_steps_per_epoch, sr_num_epochs, sr_num_valid_samples)
    return model


def show_me_sr():
    img = read_image(images_for_super_resolution()[9], 1)
    ...
    sr_model = learn_super_resolution_model(5, quick_mode=False)
    restored_img = restore_image(corrupted_img, sr_model)
...
Answer1 = "the way i implemented 'super_resolution_corruption' function is by taking the input image, shrinking it by some factor between 2-4, and than stretching it back by the same factor. this action results in an image with low resolution, i.e. less information than the original picture. this affect is caused by the fact that when i shrunk the image, inevitably, i lost information that could not be restored by simply stretching it back. so when i stretched it back, i got a picture of the same size as the original but with less information - this appears as low resolution.  the way i implemented 'learn_super_resolution_model' is very similar to the way i implemented all the other learning functions with the only difference of the corruption function being the one i described before. giving the model a corruption function, that imitates well the actual corruption of the test images, will give us a model that knows the behaviour of real LR corrupted images and thus will perform better when restoring LR images."  # @param {type:"string"}
...
Answer2 = "1)  first try:         super_resolution_num_res_blocks = 10         batch_size = 100         steps_per_epoch = 1500         num_epochs = 3         patch_size = 32         num_channels = 50          visual results: poor...         validation loss: 0.0043  2) second try: more epochs - checking if it will significantly improve the visual results?         super_resolution_num_res_blocks = 10         batch_size = 100         steps_per_epoch = 1500         num_epochs = 10         patch_size = 32         num_channels = 50          visual results: nice!         validation loss: 0.0034  3)  third try: less steps per epoch - it will take less time but will it defect the visual results significantly?         super_resolution_num_res_blocks = 10         batch_size = 100         steps_per_epoch = 100         num_epochs = 10         patch_size = 32         num_channels = 50          visual results: visually looks pretty much like the second try         validation loss: 0.0039  t chosen parameters: (second try)         super_resolution_num_res_blocks = 10         batch_size = 100         steps_per_epoch = 1500         num_epochs = 10         patch_size = 32         num_channels = 50"  # @param {type:"string"}
...
def plot_my_plots():
    # deblurr
    for num_res_blocks in range(1, 6):
        print("\n - - - - - deblurring: number of blocks: " + str(num_res_blocks) + "- - - - - \n")
        model = learn_deblurring_model(num_res_blocks, quick_mode=False)
        plt.plot(np.arange(len(model.history.history['val_loss'])) + 1, model.history.history['val_loss'],
                 label=str(num_res_blocks) + " resblocks")
        print("models\model_deblur_" + str(num_res_blocks) + "_resblocks")
    plt.legend()
    plt.title("validation loss of deblur NN \n as function of number of resblocks")
    plt.xlabel("epochs")
    ...
    # denoise
    for num_res_blocks in range(1, 6):
        print("\n - - - - - denoising: number of blocks: " + str(num_res_blocks) + "- - - - - \n")
        model = learn_denoising_model(num_res_blocks, quick_mode=False)
        plt.plot(np.arange(len(model.history.history['val_loss'])) + 1, model.history.history['val_loss'],
                 label=str(num_res_blocks) + " resblocks")
    plt.legend()
    plt.title("validation loss of denoise NN \n as function of number of resblocks")
    plt.xlabel("epochs")
    plt.ylabel("validation loss")
    plt.show()
...
Answer3 = "1)  number of epochs:     As it is shown in the graphs - the validation loss decreases as the number of epochs increases.     This happens in all NN: denoising NN and deblurring NN, with 1/2/3/4/5 resblocks.     this makes sense since more epochs mean more batches to execute gradient descend on,     therefor more steps to minimize the training loss, and as a result also the validation loss.  2)  number of resblocks:     both in the deblur NN and the denoise NN, we can see that NN that had more resblocks performed better:     deblur:     deblur NN  with 1 resblock achieved validaion loss of ~ 0.014     while deblur NN with 5 resblock achieved validation loss of ~ 0.008     denoise:     denoise NN  with 1 resblock achieved validaion loss of ~ 0.0022     while denoise NN with 5 resblock achieved validation loss of ~ 0.0020  3)  qualitative description of the images:     the less resblocks the deblurr NN had, the more blurred it's output was     the less resblocks the denoise NN had, the more noised it's output was  4)  deblurring vs. denoising:     in terms of validation loss - the denoising NN performed better achieving lower validation loss.     but, in terms of how the image was restored visualy, the restored images of the deblur NN  look much better, and      much more similar to the original pictures, i think a possible explanation for this is the fact that blurring is     a corruption that is linear- it can be done by convolution, while denoising is not linear, and that's why the      rdeblurred image looked much better than the denoised image."  # @param {type:"string"}
