...
def images_for_denoising():
    ...
    images_folder = '/Users/ellaby/Documents/year D/image_processing/exes/ex5-ellaby/datasets/image_dataset/train'
    onlyfiles = [f for f in listdir(images_folder) if isfile(join(images_folder, f))]
    files = [images_folder + "/" + file for file in onlyfiles]
    return files
...
def images_for_deblurring():
    ...
    images_folder = '/Users/ellaby/Documents/year D/image_processing/exes/ex5-ellaby/datasets/text_dataset/train'
    onlyfiles = [f for f in listdir(images_folder) if isfile(join(images_folder, f))]
    files = [images_folder + "/" + file for file in onlyfiles]
    return files
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            bigger_patch_currapted = corruption_func(img[top_y: top_y + (3 * crop_size[0]), left_x: left_x + (3 * crop_size[1])])
...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    model.fit_generator(generator=train_data_generator, steps_per_epoch=steps_per_epoch, epochs=num_epochs,
                        validation_data=validation_data_generator, validation_steps=num_valid_samples//batch_size)
...
def restore_image(corrupted_image, base_model):
    ...
    clean_image = new_model.predict(corrupted_image)[0] + 0.5
...
**Note:** In your submission, the names of the plots should be `depth\_plot\_depth_plot_denoise.png` and  `depth\_plot\_deblur.png` and they should be located in the same directory as the rest of your files.
"""
...
def plot_my_plots():
    ...
    for num_res_blocks in range(1, 6):
        ...
        plt.plot(np.arange(len(model.history.history['val_loss'])) + 1, model.history.history['val_loss'],
                 label=str(num_res_blocks) + " epochs")
    ...
    for num_res_blocks in range(1, 6):
        ...
        plt.plot(np.arange(len(model.history.history['val_loss'])) + 1, model.history.history['val_loss'],
                 label=str(num_res_blocks) + " blocks")
    plt.legend()
...
if __name__ == '__main__':
    ...
    sr_model = learn_super_resolution_model(5, quick_mode=True)
